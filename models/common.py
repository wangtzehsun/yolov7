...
class seresBottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, groups=1, base_width=64, dilation=1, norm_layer=None,
        ...
        if downsample:
            self.downsample = nn.Sequential(conv1x1(inplanes, planes * self.expansion, stride),
...
class seresLayer(nn.Module):
    def __init__(self, c1, c2, n=1, s=1, g=1, w=64,
                 downsample=False):  # chin, plane, block_nums, group, width_per_group
        ...
        for _ in range(n - 1):
            blocks.append(
                seresBottleneck(inplanes=c2 * seresBottleneck.expansion, planes=c2, stride=1, groups=g, base_width=w))
...
class DepthwiseSeparableConv2d(nn.Module):
    def __init__(self, c1, c2, k=3, s=1, p=None):
        ...
        # Depthwise Convolution
        self.depthwise = nn.Conv2d(c1, c1, kernel_size=k, stride=s, padding=autopad(k, p), groups=c1)
        self.depthwise_bn = nn.BatchNorm2d(c1)
        self.depthwise_relu = nn.ReLU(inplace=True)

        # Pointwise Convolution
        self.pointwise = nn.Conv2d(c1, c2, kernel_size=1, stride=1, padding=0)
        self.pointwise_bn = nn.BatchNorm2d(c2)
